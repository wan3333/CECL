...
def generate_noise_matrix(noise_type, closeset_noise_ratio, openset_noise_ratio=0.2, nb_classes=10):
    """
...
class NoisyCIFAR100(dsets.CIFAR100):
    def __init__(self, root, train=True, transform=None, target_transform=None, download=False,
        ...
        if self.train and (noise_type != 'clean'):
            ...
            # self.noisy_labels = new_targets
            # self.ground_truth_labels = new_targets

            true_labels = np.asarray([[self.targets[i]] for i in range(len(self.targets))])
            noisy_labels, self.actual_noise_rate = noisify_dataset(self.nb_classes, true_labels, noise_type, closeset_ratio,
            ...
            self.data = self.data[ch1]
            self.noisy_labels = r1[ch1]
            true_labels = np.array(self.targets)[ch1]
            self.ground_truth_labels = np.array(self.targets)[ch1]
            ...
            # 记录每个样本是否为噪声
            self.noise_or_not = np.transpose(self.noisy_labels) == np.transpose(true_labels)
...
class NoisyCIFAR10(dsets.CIFAR10):
    def __init__(self, root, train=True, transform=None, target_transform=None, download=False,
                 noise_type='clean', closeset_ratio=0.0, openset_ratio=0.2, random_state=0, verbose=True):
        super().__init__(root, train=train, transform=transform, target_transform=target_transform,
                         download=download)
        ...
        if self.train and (noise_type != 'clean'):
            ...
            res = torch.load('./res_stage1/res_cifar100_symmetric_0.2.pkl')
            r1 = res['r1'].numpy()
            ch1, ch2 = res['ch1'], res['ch2']
            ch = (ch1 == ch2) * (ch1 == True) * (ch2 == True)

            self.data = r1[ch].numpy()
            self.noisy_labels = r1[ch].numpy()
